...
def plot_ryjo(nc_file: str):  #pylint: disable=R0915  too-many-statements
    """
    John's plotting function.
    Minimally adjusted.
    """
    ...
    # plotting variables
    psdlabl = (
        r'Spectrum level (dB re 1 " r"$\mu$Pa$\mathregular{^{2}}$ Hz$\mathregular{^{-1}}$)'
    )
    ...
    # for output file replace nc_file extension to jpg:
    jpeg_file = nc_file.replace(".nc", ".jpg")
    plt.savefig(jpeg_file, dpi=200)
    # plt.show()

from pbp.job_agent.main_job_agent_args import parse_arguments # This module is used to parse the arguments provided by the user.
from multiprocessing import Process # This module is used to create allow multiple processes.
...
def main(): # This function represents the entry point for the job agent.
    ...
    from pbp.job_agent.job_agent import JobAgent # This module is used to create the job agent object.
    processes = [] # This list is used to store the processes created by the job agent.
    for deployment_configuration in deployment_configurations: # Iterates through the deployment configurations provided by the user.
        ...
        pbp_job_agent_process = Process(target = job_agent.run) # Creates a process for the job agent.
        processes.append(pbp_job_agent_process) # Appends the process to the list of processes.
    ...
    for process in processes: # Iterates through the processes.
        try: # Tries to join the process.
            ...
            print(f"Operation was interrupted by the user: {error}") # Prints the error message.
